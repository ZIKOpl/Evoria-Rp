...
// ── SESSION UTILISATEUR (localStorage uniquement pour la session) ─
const SESSION = {
  get user()   { return JSON.parse(localStorage.getItem('evr_user')   || 'null'); },
  setUser(v)   { localStorage.setItem('evr_user', JSON.stringify(v)); },
  clearUser()  { localStorage.removeItem('evr_user'); },
  async isAdmin() {
    const u = this.user;
    if (!u || !u.id) return false;
    // Vérification live auprès du serveur
...
async function fetchDiscordMembers() {
  ...
  if (!elSD && !elBD) return;
  if (typeof CONFIG === 'undefined' || !CONFIG.discordGuildID) return;
  try {
    const res = await fetch(`https://discord.com/api/v10/guilds/${CONFIG.discordGuildID}/widget.json`, { signal: AbortSignal.timeout(6000) });
    if (!res.ok) throw new Error();
    const d = await res.json();
    const online = d.presence_count ?? 0;
    if (elSD) animNum(elSD, online, 1200);
    if (elBD) animNum(elBD, online, 1200);
  } catch {
...
function updatePreview() {
  ...
  const tagColor = document.getElementById('news-tag-color')?.value || '#A78BFA';
...
function handleOAuthCallback() {
  ...
  if (!isAdmin) { toast(`Connecté — vous n'êtes pas admin.`, ''); return; }
